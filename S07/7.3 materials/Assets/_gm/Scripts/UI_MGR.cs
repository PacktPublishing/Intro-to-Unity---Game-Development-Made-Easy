...
public class UI_MGR : MonoBehaviour
{
    ...
    public GameObject _finalTitlesGO;


    public void ShowGameOverScreen(bool isShow){
        _gameOverGO.SetActive(isShow);
    }

    public void ShowFinalTitles(bool isShow){
        _finalTitlesGO.SetActive(isShow);
    }
    ...
    void Update(){
        ...
        if (Input.GetKey(KeyCode.F2)){
            ShowFinalTitles(true);
        }
    }

    void Start(){
        _gameOverGO.SetActive(false);
        _finalTitlesGO.SetActive(false);
